...
async function validateTextDocument(textDocument) {
  ...
  errors.forEach((error, i) => {
    ...
    if (
      ...
      error.includes(
        "Error: Element “div” not allowed as child of element “b” in this context."
      )
    ) {
      // 4.1.1.3
      error = error.replace(/[\u201C\u201D]/g, '"');
      var elems = error.match(/"([^"]+)"/g);

      errorMsg = "Improper element nesting, invalid child elements.";
      suggestMsg = `Please refrain from using ${elems[0]} as a child element of ${elems[1]}.`;
      source = "WCAG 2.1 | 4.1.1.3";
    } else {
      return;
    }
